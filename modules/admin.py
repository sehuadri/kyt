...
async def add_ip(event):
	async def add_ip_(event):
		...
		bz = f" curl -sS https://raw.githubusercontent.com/RMBL- ZERO/permission/main/ipmini | grep '{ox}' | cut -d ' ' -f7 "
...
async def add_key(event):
	async def add_key_(event):
		ox2 = "VIP"
		ox = requests.get(f"https://ipv4.icanhazip.com").text.strip()
		bz = f" curl -sS https://raw.githubusercontent.com/casper9/permission/main/ipmini | grep '{ox}' | cut -d ' ' -f7 "
		bo = subprocess.check_output(bz, shell=True).decode("ascii").strip()
		if not ox2 != bo:
			async with bot.conversation(chat) as exp:
				await event.respond(f"""
...
[Button.inline("KEY GENERATOR","add7-key2")]]
	ox2 = "ON"
	ox = requests.get(f"https://ipv4.icanhazip.com").text.strip()
	bz = f" curl -sS https://raw.githubusercontent.com/casper9/permission/main/ipmini | grep '{ox}' | cut -d ' ' -f5 "
	bo = subprocess.check_output(bz, shell=True).decode("ascii").strip()
	if not ox2 != bo:
		sender = await event.get_sender()
...
"""
		...
		await event.respond(f"**Your Not Admin Chat @rmblvpn1 For buy Key**")
